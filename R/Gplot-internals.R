...
# init.pos              : "random" randomize nodes on a circle.
#                         "cercle" directly on the circle.
...
# ------------------------------------------------------------------------
  ...
  graph$network$vertex.reso = 10
  graph$network$edge.reso   = 0.5
  graph$network$arrow.reso  = 10
    ...
    if ( draw.pie ) {
      ...
      for (i in 1:n){
        ...
        if ( is.nan( sector.theta[1] ) ) {
          ...
          draw.cercle( graph, radius=v.radius[i], sides=-1,
      ...
      draw.cercle( graph, radius=v.radius, sides=-1, center.x=x, center.y=y ,
                  col=v.col, border=v.border, lty=v.lty, ...  )   
    }
  }
  return( graph )
  ...
  n <- dim( graph$mat )[1]
   ...
   if( (length(l.px0)>0) & loop.draw ){
     ...
     draw.loops( graph, 
                ...
                a.len       = 2 * pmax( l.e.lwd * scale.edge, scale.arrow) ,
                a.angle     = -1,
